function plot_data(arg)
    ...
    % Plot data versus magnet temperature
    xFieldName = fieldNames.temperatureMagnet;
    yQuantities = {...
        'transportX', ...
        'transportY'};
    num = numel(yQuantities);
    for i=1:num
        ...
        currentTab = uitab(tg, 'Title', [char(yFieldName), '-Mag']);
        ax = axes(currentTab);
        plot(ax, logdata.(xFieldName), logdata.(yFieldName), ...
            '.', 'MarkerSize', 20);
        xlabel(ax, 'Magnet Temperature, K');
        ylabel(ax, yFieldName);
        grid(ax, 'on');
    end
    ...
    yQuantities = {...
        ...
        'temperature', ...
        'temperatureMagnet', ...
        'temperatureColdplate', ...
        'temperatureRadShield'};
    num = numel(yQuantities);
    for i=1:num
        yFieldName = fieldNames.(yQuantities{i});
        if isempty(yFieldName)
